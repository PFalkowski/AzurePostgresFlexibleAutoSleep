...
2. **`WakeOnStartup` + `StopOnShutdown` can thrash.** With both on, every cold start wakes the DB and the next scale-in stops it. On true scale-to-zero, prefer `StopOnShutdown` *without* `WakeOnStartup` (let the wake middleware bring the DB up lazily on the first real request). Document this; consider a startup warning when both are set.
